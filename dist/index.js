...
function createApiClient(siteURL) {
    ...
    return {
        ...
        async addToCart(cart_key = "", id, quantity = "1") {
            try {
                ...
                if (typeof id !== "string")
                    throw new Error(" id is required as `string`.");
                if (id === "")
                    throw new Error("id is required, can't be empty.");
                // Ensure quantity is provided as `string`
                if (typeof quantity !== "string")
                    throw new Error("quantity is required as a `string`");
        ...
        async updateItem(cart_key, item_key, quantity) {
            try {
                ...
                if (typeof quantity !== "string")
                    throw new Error("quantity is required as `string`.");
                if (quantity === "")
